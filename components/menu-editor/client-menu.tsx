...
import { formatMenuPrice, languages as supportedLanguages, translateText, uiCopy, type LanguageCode } from '@/lib/menu-i18n'
...
export function ClientMenu() {
  ...
  const logoUrl = style.logoUrl || '/logo.svg'
  const showingFavorites = activeCategory === 'favorites'

  useEffect(() => {
    const savedUserRatings = window.localStorage.getItem('rinconcito-product-user-ratings')

    try {
      if (savedUserRatings) setUserRatings(JSON.parse(savedUserRatings) as Record<string, number>)
    } catch {
      window.localStorage.removeItem('rinconcito-product-user-ratings')
    }
  ...
  const rateItem = async (itemId: string, rating: number) => {
    ...
    window.localStorage.setItem('rinconcito-product-user-ratings', JSON.stringify(nextUserRatings))
      ...
      <section className="relative overflow-hidden px-4 py-8 text-white sm:py-10">
        ...
        <div className="relative mx-auto max-w-5xl">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <img src={logoUrl} alt="El Rinconcito Español" width={56} height={56} loading="eager" decoding="async" fetchPriority="high" className="h-14 w-14 rounded-full bg-white object-cover ring-2 ring-white/50" />
